...
ADD_USER_TO_GROUP = f"http://{CONFIG_IP}:{CONFIG_PORT}/add_user_to_group"
GET_USER = f"http://{CONFIG_IP}:{CONFIG_PORT}/users"
GET_GROUP = f"http://{CONFIG_IP}:{CONFIG_PORT}/groups"
...
def assign_user_to_group(user_id, group_id):
    ...
    try:
        ...
        response = requests.post(ADD_USER_TO_GROUP, headers=headers, json=data)
        response.raise_for_status()
...
def fetch_existing_users():
    try:
        ...
        response = requests.get(GET_USER, headers={'accept': 'application/json'})
        response.raise_for_status()
...
def fetch_existing_groups():
    try:
        ...
        response = requests.get(GET_GROUP, headers={'accept': 'application/json'})
        response.raise_for_status()
