    ...
    class AddUserToGroup(Resource):
        ...
        def post(self):
            try:
                ...
                db.session.rollback()  # Rollback the session to avoid locking
                logging.error(f"IntegrityError: {str(ie)}")
                return {'message': 'User is already in the group'}, 400  # Return a 400 status for this specific case
            except Exception as e:
                db.session.rollback()
                logging.error(f"Error adding user to group: {str(e)}")
                return {'message': 'Error adding user to group'}, 500
    ...
    class AddUserToGroup(Resource):
        ...
        def post(self):
            try:
                ...
                db.session.add(group_task)
                db.session.commit()
                return {"message": "Task added to group successfully!"}, 201
            except IntegrityError as ie:
                db.session.rollback()  # Rollback the session to avoid locking
                logging.error(f"IntegrityError: {str(ie)}")
                return {'message': 'task is already in the group'}, 400  # Return a 400 status for this specific case
            except Exception as e:
                db.session.rollback()
                logging.error(f"Error adding task to group: {str(e)}")
                return {'message': 'Error adding task to group'}, 500
